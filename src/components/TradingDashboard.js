...
const toggleHistory=()=>{
  ...
  return (
    <div className='w-full h-max	  sm:w-full sm:px-[4rem]  mt-4 sm:mt-0 bg-black  overflow-x-hidden '>
...
<div className='w-full h-full  flex flex-col lg:flex-row lg:mt-4 mt-14  '>
  ...
  className='lg:w-2/6 h-screen flex flex-col gap-2 mr-2 '>
...
{/* 
 <div className="hs-dropdown relative bg-white" >
      <button 
        id="hs-dropdown-default" 
        type="button" 
        className="w-full hs-dropdown-toggle py-3 px-4 inline-flex items-center gap-x-2 text-lg font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
       
        onClick={toggleHistory}
      >
       Today's Trade History
        <svg className={isOpen ? "rotate-180 w-4 h-4" : "w-4 h-4"} 
             xmlns="http://www.w3.org/2000/svg" 
             width="24" 
             height="24" 
             viewBox="0 0 24 24" 
      ...
      <div 
  className={`hs-dropdown-menu transition-[opacity,margin] duration-200 ${isOpen ? 'opacity-100 visible max-h-[20rem] overflow-y-auto' : 'opacity-0 invisible max-h-[0rem] overflow-y-hidden'} min-w-[25rem] bg-black shadow-md rounded-lg p-2 mt-2 dark:bg-gray-800 dark:border dark:border-gray-700 dark:divide-gray-700 after:h-4 after:absolute after:-bottom-4 after:start-0 after:w-full before:h-4 before:absolute before:-top-4 before:start-0 before:w-full`}
  aria-labelledby="hs-dropdown-default"
>
       {showData && tradeData ? ( // Only display if showData is true and tradeData exists
